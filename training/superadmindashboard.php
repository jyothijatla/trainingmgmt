...
($_SESSION['usertype_id'] == 1) {
    ...
    $sqladmin = "SELECT `admin_id` FROM `admin_master` ";

    $resultadminmaster= mysqli_query($conn, $sqladmin);

    $admin = mysqli_num_rows($resultadminmaster);
    ...
    $sqlcourse = "SELECT `course_id` FROM `institute_category`";

    $rescou = mysqli_query($conn, $sqlcourse);

    $course = mysqli_num_rows($rescou);

    $sqlbranch = "SELECT `branch_id`
FROM `branch_master` ";

    $resbrch = mysqli_query($conn, $sqlbranch);

    $branch = mysqli_num_rows($resbrch);
...
<html>
        ...
        <body>
            ...
            <h1 style=text-align:center><b>Super Admin Dashboard<b></h1>
        ...
        <br><a href="superadminpage.php">Connect to SuperAdminPage</a>
        ...
        <div class='staff'>
            ...
            <div class='admin'>
                <div class ='pad'>Admin <?php echo $admin ?></div>
            </div>

            <div class='branch'>
                <div class ='pad'>Branches <?php echo $branch ?></div>
            </div>
  ...
  <div>
    <canvas id="myChart"></canvas>
            ...
            type: 'pie',
            data: {
            labels: ["staff", "branches", "courses", "students", "admin"],
            datasets: [{
            backgroundColor: [
            "#2ecc71",
            "#3498db",
